...
final class OrderStore: ObservableObject {
    @Published var order: OrderInfo = OrderInfo(orderId: "", orderedUserInfo: "", orderTime: Timestamp(date: Date.now), orderedItems: [], orderAddress: "", orderMessage: "", payment: .byCreditCard)
...
//        guard let currentUserUid else { return }
    ...
    //FIXME: 고객의 구매 정보를 스토어에 넘기는 함수 구현 중... 
    public func createOrderedItemInfo(with currentUserUid: String?,
        ...
        for orderedItem in item {
            let orderedItemsInfo = OrderedItemInfo(itemUid: orderedItem.itemId, itemName: orderedItem.itemName, itemImage: orderedItem.itemImage, price: orderedItem.price, option: orderedItem.itemAllOption, deliveryStatus: DeliveryStatusEnum.pending)
            orderedItemsArray.append(orderedItemsInfo)
        }
        
        // 주문 정보 생성
        let newOrderInfo = OrderInfo(orderId: UUID().uuidString, orderedUserInfo: currentUserUid, orderTime: Timestamp(date: Date.now), orderedItems: orderedItemsArray, orderAddress: "배송주소", payment: PaymentEnum.byAccount)
